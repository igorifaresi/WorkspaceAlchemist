...
Texture2D<float4> ext_textures[8] : register(t0);
...
float4 PsMain(pixeldesc pixel) : SV_TARGET
{
	...
	&&  pixel.screen_pos.y >= pixel.clip_vec.y && pixel.screen_pos.y <= pixel.clip_vec.w) {
		...
		if (pixel.flags == 2) {
			...
			case 0: { return ext_textures[0].Sample(default_sampler, pixel.uv) * pixel.color; } break;
			case 1: { return ext_textures[1].Sample(default_sampler, pixel.uv) * pixel.color; } break;
			case 2: { return ext_textures[2].Sample(default_sampler, pixel.uv) * pixel.color; } break;
			case 3: { return ext_textures[3].Sample(default_sampler, pixel.uv) * pixel.color; } break;
			case 4: { return ext_textures[4].Sample(default_sampler, pixel.uv) * pixel.color; } break;
			case 5: { return ext_textures[5].Sample(default_sampler, pixel.uv) * pixel.color; } break;
			case 6: { return ext_textures[6].Sample(default_sampler, pixel.uv) * pixel.color; } break;
			case 7: { return ext_textures[7].Sample(default_sampler, pixel.uv) * pixel.color; } break;
			}
		}
